...
export interface DigiProductDetail extends DigiProduct {
  description?: string; // raw HTML including <delivery> and <attention> tags
}
...
// Strip HTML for short previews only (product cards)
...
export async function getSellerProducts(
  ...
  try {
    ...
    if (data.retval !== '0' && data.retval !== 0) {
      console.error('[Digiseller] API error:', data.retval, data.retdesc);
      return { products: [], total: 0, totalPages: 0 };
    }
...
export async function getProductDetail(
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (item && (item.id || item.id_goods)) {
        ...
        return {
          ...
          // Keep raw HTML for full description page — includes <delivery> and <attention> tags
    ...
    // Fallback: find in listing and fetch info raw
    const { products } = await getSellerProducts(1, 200, currency, lang);
    const found = products.find((p) => p.id === productId);
    if (found) {
      // Re-fetch the raw product from the listing to get full info HTML
      ...
      if (listRes.ok) {
        ...
        if (raw) {
          return {
            ...found,
            // raw info HTML preserved for description page
...
export async function getProductReviews(
  ...
  try {
    const url = `${API_BASE}/reviews/product?id_goods=${productId}&type=1&owner_id=${SELLER_ID}&lang=${lang}&page=1&rows=20`;
    const res = await fetch(url, {
      headers: { Accept: 'application/json' },
      next: { revalidate: 600 },
    });
